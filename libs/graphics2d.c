...
void drawImageEasy(Image* img, int x, int y)
{	
	sceGumPushMatrix();
	...
	sceGuTexImage(0,img->texWidth,img->texHeight,img->texWidth,img->data);
...
void drawImageR(Image* img, int x, int y, float radians)
{
	sceGumPushMatrix();
	
	ScePspFVector3 loc = {x+img->width/2,y+img->height/2,0.0f};
	sceGumTranslate(&loc);
	if(radians!=0.0f)sceGumRotateZ(radians);
...
void drawImageA(Image* img, int x, int y, u8 alpha)
{
	sceGumPushMatrix();
...
void drawImageRA(Image* img, int x, int y, float radians, u8 alpha)
{	
	sceGumPushMatrix();
